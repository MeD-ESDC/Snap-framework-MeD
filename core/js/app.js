define([
...
], function (_, $, labels, CoreSettings, Utils, Logger, History, Router, Legacy, popperJs, MicroLearning, ScormController, DataController, Regular, LockingSystem, ObjNiveau, ObjSub, ObjSubUtils, PrintController, MobileInteractions, AdminMode, Navigation, Timeline, HelpController, SitemapController, Toolbox, Glossary, Resources, FavoriteController, ProgressController, googleAnalytics) {
	'use strict';
	...
	function navStructure() {
		this.version = "1.6.3"; //*DO NOT MODIFY. Auto-generated by build*//
		this.WETversion = "4.0.26"; //*DO NOT MODIFY. Auto-generated by build*//
		...
		*/
		...
		this.initModules = function () {
			...
			if (CoreSettings.activateTimeline) {
		...
		*/
		...
		this.runAutomations = function () {
			...
			if ($(CoreSettings.contentContainer).find(".qs-elearning-activity").length > 0) {
				$.getScript(require.s.contexts._.config.baseUrl + "plugins/qs/qsbeta.js");
			}
				...
				if (mfp.st.type == 'ajax') {
					...
					$(this).on("mfpAjaxContentAdded", function (e, mfp) {
						...
						if (!scanDone) {
							scanDone = true;
							var $el = mfp.content;
							masterStructure.resources.scan($el);
							that.initWbs($el);
						}
					});
			...
			$(CoreSettings.contentContainer).on("wb-contentupdated", function (event, data) {
				masterStructure.resources.scan($(data.content));
				masterStructure.glossary.scan($(data.content));
		...
		this.loadFailed = function (xhr) {
			this.isLoaded();
			//sub might have already been detected by the diagnosis as a missingPage,
			...
			$(this.adminMode).trigger('Diagnosis:addMissingPage', [this.currentSub, xhr]);
			Utils.hideLoadingBox();
		};
		...
		this.disableBacknext = function () {
			...
			if (this.isLastPage() && CoreSettings.loopLevel >= this.maxDepth) {
				$(".next").addClass("disabled").attr({ "tabindex": "-1", "aria-disabled": "true" });
		...
		this.getFirstPage = function () {
			...
			var bookmark = (this.scorm.getBookmark()) ? this.scorm.getBookmark() : "";
			//just a flag
