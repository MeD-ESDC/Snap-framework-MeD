define(['jquery', 'settings-core'], function($, CoreSettings) {
   ...
   return {
      ...
      setAriaSetsize: function(obj) {
	   ...
	   //send an object, an array of classes and the chosen class
	  classToggle:function($el, aClasses, chosenClass){
		  for (var i=0;i<aClasses.length;i++){
			  $el.eq(0).removeClass(aClasses[i]);
		  }
		  $el.eq(0).addClass(chosenClass);
